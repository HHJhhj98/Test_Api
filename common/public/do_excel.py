...
from common.public.do_regx import DoRegx
...
class DoExcel:
    ...
    def read_excel(self, filename):
        ...
        for key in mode:  # 遍历存在配置文件里面的字典
            ...
            self.update_tel(NoRegTel + 3, filename, 'init')
        for item in test_data:
            if item['data'].find('${tel_1}') != -1:
                item['data'] = item['data'].replace('${tel_1}', str(NoRegTel))
            elif item['data'].find('${tel_2}') != -1:
                item['data'] = item['data'].replace('${tel_2}', str(NoRegTel + 1))
            elif item['data'].find('${tel_3}') != -1:
                item['data'] = item['data'].replace('${tel_3}', str(NoRegTel + 2))
            else:
